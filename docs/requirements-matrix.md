...
| RCI-074 | Question repertoire succession is typed, inert, attacked, and independently admitted | verified G3R; implemented G3Q candidate, hosted gate pending | Existing exact admission plus the derived `CompiledQuestionContract` registry, exact context/policy pins, ordinary scheduling, provisional return routing, and regenerative acceptance attacks |
...
| G3Q | Implemented local candidate for confined regenerative question scheduling; hosted gate pending | No arbitrary code, general planner, source authority, or automatic warrant |
