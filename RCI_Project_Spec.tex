...
Inquiry (RCI) version 0.6. It preserves the v0.4 aggregate/history correction and v0.5
recursive-project authority discipline, and is a
...
\toprule
Source & Role in 0.6 & Reconciliation \\
\midrule
\endhead
...
Derivations for the succession--arrangement kernel &
Their typed effects, arrangements, open relations, answer frames, and bounded
recognition are normalized through ADR-0018 after two owned incumbent expressivity
failures. Candidate completions remain non-truths; semantic completion remains distinct
from external return; and generated syntax retains no execution or authority. \\
\bottomrule
\end{longtable}
...
\section{Succession--arrangement compositional kernel}

For a typed return interface $A$, a represented effect has type
$e:\mathsf{Eff}[A]$. Actualization produces a dependent pair
$\langle e,a\rangle$ with $a:A$. A binding-derived realized interaction trace is linear:
\[
 \tau ::= \varepsilon \mid \tau\mathbin{\cdot_S}\langle e,a\rangle.
\]
It is not ledger order, aggregate state, arbitrary event history, or a universal
occurrence carrier. Represented effect, persisted request, attempt, raw return, decoded
result, and checked observation remain distinct.

An arrangement $\alpha:\mathsf{Arr}[I,O]$ is perpendicular to succession. Its bounded
forms include typed identity and primitive arrangements, sequential composition
$\beta\circ_A\alpha$, typed parallel composition, finite hiding with reopening
provenance, finite case arrangements, typed crossing/permutation, preservation-checked
transport, and bounded structural re-entry. Normatively,
\[
 \circ_A\neq\cdot_S.
\]
Composition builds a represented arrangement; succession records what was realized.

The canonical interaction-program relation is
\[
 P ::= \operatorname{Ret}(z)\mid\operatorname{Vis}(e,\kappa),
 \qquad \kappa:A\to P,
\]
with transition
\[
 \operatorname{Vis}(e,\kappa)\xrightarrow{\langle e,a\rangle}\kappa(a).
\]
The executable repository realization MUST be a finite typed guarded graph; every cycle
contains a new-return boundary and epsilon cycles are forbidden.

A question $q:\mathsf Q[A(q)]$ specializes the effect family through
$\operatorname{Ask}(q):\mathsf{Eff}[A(q)]$. It owns stable interrogative identity,
answer interface, applicability/comparison semantics, and renderer. An optional
relational lowering $\operatorname{Lower}_R(q)=U_q$ does not identify $q$ with $U_q$.
Discharge authority remains external to question identity.

The static relational source language retains truth, equality, registered relation
application, negation, conjunction, and finite existential hiding. Open ports are typed
compiler objects. Candidate completions are elements of a declared completion carrier,
not asserted members of a solution fiber. A decoded observation may leave multiple
answer cells live; applicability exterior, malformed decode, checker outcome, and
operational failure remain outside semantic sibling cells.

For a closed exhaustive binary answer interface, case arrangement specializes to
Boolean distinction. Crossing by swap is involutive and projects extensionally to field
complement. Conjunction, disjunction, implication, and equivalence are deterministic
surface macros. No Boolean surface form erases succession order.

Finite owned trace fragments may support an inert arrangement candidate through a
binding-specific recognition relation. Recognition does not establish actuality or
warrant. A later fragment may break the candidate and reopen any fold. Persistence of
an arrangement across a succession is a protected transport/commutation claim, not a
state bit, and requires independent checking under the declared horizon.
...
\paragraph{RCI-080 --- Typed effects and realized succession.}
Every represented effect MUST declare one return interface. Realized interaction MUST be
an ordered typed effect/return pair in a binding-derived linear trace. Ledger order,
represented effect, request, attempt, return, decode, check, and realized succession
MUST NOT collapse.

\paragraph{RCI-081 --- Typed arrangement algebra and Boolean specialization.}
Arrangement identity, sequential/parallel composition, finite hiding, case, crossing,
transport, and bounded re-entry MUST preserve exact interfaces. Arrangement composition
MUST remain distinct from realized succession. Closed exhaustive binary crossing MUST be
involutive and project losslessly to legacy Boolean complement semantics.

\paragraph{RCI-082 --- Relational lowering and open ports.}
The finite typed relational language MUST fix composition direction, bound/open port
identity, and hiding provenance. A relational question lowering is optional and
conservative; it MUST NOT identify a question with its open relation or a candidate
completion with truth.

\paragraph{RCI-083 --- Typed questions and checked answer frames.}
Questions MUST declare answer interfaces and antecedent finite frames. Sibling cells are
exclusive only inside their applicability field. Partial observations MAY leave several
cells live; exterior, malformed, checker, unsupported, timeout, failure, and Unknown
outcomes remain stage-distinct. Branch narrowing requires the declared independent check.

\paragraph{RCI-084 --- Answer-conditioned guarded continuation.}
A checked answer MAY select a typed successor program through a finite response-indexed
continuation table. Every directed cycle MUST cross an effect whose resolution requires
a new return. Interpretation emits only inert actions and MUST NOT execute, admit,
check, warrant, review, or promote itself.

\paragraph{RCI-085 --- Piecewise recognition, persistence, fold, and reopening.}
Owned ordered trace fragments MAY support an inert arrangement candidate. Recognition
MUST preserve fragment provenance and survive independent discriminators before fold or
operator admission. Persistence across succession requires checked protected transport;
a breaking fragment or failed transport MUST reopen the fold or return Unknown.

\paragraph{RCI-086 --- Conservative authority and replay compatibility.}
G3K implementations MUST preserve every RCI-001--RCI-079 disposition, sealed event byte
and meaning, aggregate/history/state distinction, core-v1 behavior, and admission/warrant
boundary. New durable grammar ownership uses additive version-1 events and a rebuildable
folded-state version. Generated or recognized structures remain inert until separately
admitted, and deterministic replay invokes no effect.
...
\item \path{docs/goals/}: bounded milestone targets, including sealed predecessors,
      stopped candidates, and the active G3K-S semantic-normalization target;
